...
//        StdDraw.setPenRadius();
        filledRectangleArr(new double[]{20, 30, 40, 50, 60, 40}, new int[]{2, 3});
     ...
     *
     * @param arr    指定的数组
     * @param indexs 指定索引的柱状图，颜色为黑色。
     */
    public static void filledRectangleArr(double[] arr, int... indexs) {
        setPenColor(StdDraw.BOOK_BLUE);
        double space = 0.005;
        double width = (1 - ((arr.length + 1) * space)) / arr.length;
        double x;

        for (int i = 0; i < arr.length; i++) {
            double height = arr[i] / 100;
            x = space * (i + 1) + i * width + width / 2;
            if (indexs != null) {
                for (int j : indexs) {
                    if (i == j) {
